...
def build_refine_messages(variants, instruction, style=""):
    """Текущие варианты + голосовая правка → новые варианты. Модель сама решает, сколько вернуть:
    один (если пользователь выбрал/уточняет один) или несколько в разных стилях. Правка может быть грубой."""
    rude = _is_rude(instruction) or _is_rude(style)
    rules = RUDE_RULES if rude else RULES
    cur = "\n".join(f"{i + 1}. {v}" for i, v in enumerate(variants))
    task = (
        f"Вот текущие варианты текста:\n{cur}\n\n"
        f"Пользователь голосом сказал: «{instruction}»\n\n"
        "Пойми, что именно он хочет, и верни JSON-массив строк — только итоговые варианты, без пояснений:\n"
        "- Если он выбрал или уточняет один вариант («первый», «второй», «мне нравится второй, добавь…») — "
        "верни ровно один вариант: этот, с внесённой правкой. Остальные убери.\n"
        "- Если просит сделать несколько вариантов, возможно в разных стилях («сделай два: один деловой, "
        "другой агрессивный») — верни столько, сколько просит, каждый в своём стиле.\n"
        "- Если просит изменить все — измени все и верни столько же.\n"
        "Сохраняй смысл, факты и адресата; меняй только то, о чём просят."
    )
    return [{"role": "system", "content": rules},
            {"role": "user", "content": task}]


# Переписать ВЫДЕЛЕННЫЙ текст: пользователь выделил текст в приложении, нажал хоткей и сказал
# фразу, НАЧИНАЮЩУЮСЯ со слова-маркера («правка, сделай короче и нейтральнее»). Всё после
# маркера — свободная инструкция модели. Только маркер включает режим — никаких догадок по речи,
# поэтому обычная диктовка со словом «выделенный» правку не запускает.
def selection_instruction(text, keyword=DEFAULT_SELECTION_KEYWORD):
    """Если фраза начинается со слова-маркера, вернуть инструкцию (остаток после него), иначе None."""
    t = (text or "").strip()
    if not t or not keyword:
        return None
    m = re.match(r"\s*" + re.escape(keyword.strip()) + r"\b[\s,:.!—–-]*", t, re.IGNORECASE)
    if not m:
        return None
    instr = t[m.end():].strip()
