...
# Команда должна НАЧИНАТЬСЯ с глагола-приказа и ссылаться на выделение
# («измени выделенный текст…», «сделай выделенное официальным», «исправь этот текст»).
# Просто упоминание слова «выделенный» посреди речи режим не включает.
_SELECTION = re.compile(
    r"^(?:(?:так|ну|вот|слушай|окей|ладно|давай|короче|значит)[,\s]+)*"  # ведущие вводные слова
    r"(?:измени|поменяй|замени|исправь|поправь|перепиши|переделай|переоформи|оформи|сделай|переведи|"
    r"сократи|улучши|отредактируй|дополни|дораб\w*)\b"
    r"(?:[^.!?]*?\bвыделенн\w*|"          # …выделенный/выделенное [текст]
    r"\s+(?:это|этот|тот)\b)",            # …это/этот/тот [текст]
    re.IGNORECASE)
