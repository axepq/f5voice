...
def strip_fillers(text):
    ...
    if not text:
        return text
    had_lead = bool(_FILLER.match(text.lstrip()))  # фраза начиналась с филлера?
    t = _FILLER.sub("", text)
    ...
    t = t.strip()
    # Только если срезали ИМЕННО ведущий филлер — вернуть заглавную началу предложения.
    # Иначе не трогаем регистр: «массив arr[0]», «alex@gmail.com» должны остаться как есть.
    if had_lead and t[:1].isalpha() and t[:1].islower():
        t = t[0].upper() + t[1:]
    return t
